...
`0.9.10-beta.1` 中三端的模组内“快捷键修改”页可保存并立即应用 UI、F8 AI 控制台、缩放、
导航和小游戏中心按键；默认 `M` 直接打开包含五个游戏的小游戏中心。旧版分支仍只使用
Minecraft 1.20.1 的按键、界面与加载器 API，不混入 26.2 类名。
...
- AI 管理页会实时列出 AI 的当前维度与 XYZ，并提供“传送至该 AI”按钮；传送属于服务器管理操作，需要管理员权限。F8 会打开独立 AI 控制台，列出维度/位置并允许直接选择 AI、发送内容和传送。
...
- 按 `F8` 打开或关闭 AI 控制台；AI 请求的接收、执行或错误信息直接显示在该弹窗消息区，不写入聊天栏。
- API 页面打开时会从服务器读取已持久化的地址与模型；保存成功后服务器回传同一份快照，重新进入界面不会恢复为旧默认值。API 令牌仍只保存在服务器端且永不回传客户端。
...
`0.9.10-beta.1` 起也可使用默认 `M`（可在“快捷键修改”中更改）直接打开。

...
- 导航默认 `F7`，也可从“高级设置”或“全部已完成功能”按钮进入；三个版本均直接打开可搜索、可分页的群系/结构/维度目录，不把结果写入聊天栏。
- 目录读取服务器动态注册表，因此会列出当前整合包中实际注册的模组群系、结构与维度；单次定位由服务端执行并带 5 秒冷却。
- 选择目标后在目标附近随机安全位置生成临时磁石，并向玩家物品栏加入、自动选中已绑定的磁性指南针“方向导航”。
- 左上角显示导航类型和注册表名称；顶部以类似 Boss 血条的距离进度条显示剩余方块，屏幕同时显示转向提示。
- 朝向相对目标偏差达到 60° 且距离超过 12 格时显示“您已偏航”，并给出正确八方向与转向提示。
- 距离临时磁石 8 格内视为抵达；抵达、主动取消、断线或停服时恢复磁石位置原方块并回收带所有者标记的方向导航指南针。
- 1.20.1 Fabric 使用旧版 `ResourceLocation`、Fabric 包网络和指南针 NBT；1.20.1 Forge 使用 Forge 47 `SimpleChannel` 与同版 NBT，没有混入 26.2 数据组件 API。
...
- beta.5 起默认启用；旧配置首次迁移时会启用一次，之后管理员仍可在“高级设置”中关闭。玩家可用默认 `F7` 键打开搜索界面。
- 目录来自服务器当前的群系、结构与维度注册表，包含模组注册内容。
- 选择目标会生成临时磁石及绑定的“方向导航”指南针；HUD 显示方向、目标维度、剩余方块、进度条和偏航警告。
- 抵达、取消、断线或停服都会清理临时磁石和本模组生成的指南针；26.2 传送模式仅允许管理员使用。

...
`0.5.7` 已完成设计文档中的五个小游戏。打开完整管理中心并选择“小游戏中心”即可游玩。
...
- `windowsdepcs-ai-companion-0.5.7.jar`：正式模组，放入 `mods/`。
- `windowsdepcs-ai-companion-0.5.7-sources.jar`：源码包，供开发工具使用。
- `windowsdepcs-ai-companion-0.5.7-javadoc.jar`：Java API 文档包。

普通玩家只安装第一个正式模组 JAR；不要把 sources 或 Javadoc JAR 放进 `mods/`。

## 当前边界

0.5.7 保留现有 AI 决策与任务框架，但复杂地形寻路、完整战斗/挖掘/合成/背包执行器、

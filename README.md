...
当前测试版本：`v0.9.10-beta.4`（预发行版；稳定版仍为 `v0.9.9`）
...
`0.9.10-beta.4` 起，1.20.1 模组内快捷键页采用“功能 / 开关 / 改键 / 重置”四列布局；
默认 `F9` 打开五游戏中心，缩放与导航为 `F6/F7`。旧 M/C/G 默认值会安全迁移，玩家自定义值不被覆盖。
...
| 模组 | 适用发行目标 | 用途 | 网址 |
| --- | --- | --- | --- |
| Mod Menu | Minecraft 1.20.1 Fabric | 从模组列表打开完整配置中心 | [Mod Menu](https://modrinth.com/mod/modmenu) |
| Cloth Config API | Minecraft 1.20.1 Fabric（优先推荐） | 低版本配置生态兼容；1.20.1 不使用 EclipseUI | [Cloth Config](https://modrinth.com/mod/cloth-config) |
...
### 0.9.10-beta.4 快捷键、提示词、女仆与小游戏修复

- 五个小游戏使用各自玩法，不再全部显示为反应训练。
- 恢复提示词内容编辑、中文模式/预设名称和 AI 女仆所有者任务页面。
- 仍只使用 Java 17 与 Minecraft 1.20.1 Mojang 映射 API，不混入 26.2 命名。

### 0.9.10-beta.3 UI 完整目录

- 九分类主界面的功能按钮进入独立详情页，结果保留在页面内。
- 完整目录覆盖 AI、小游戏、游戏/客户端增强、休闲、性能、兼容与高级功能。
- 1.20.1 Fabric 保持 Java 17、旧版 `Screen`/`GuiGraphics` 命名；小游戏方向、动作、暂停、重开与辅助键可编辑并持久化。
...
`0.9.10-beta.4` 起可使用默认 `F9`（可修改）直接打开五游戏入口。

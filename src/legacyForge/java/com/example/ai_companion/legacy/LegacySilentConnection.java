...
/** Unique in-memory connection for a server-controlled 1.20.1 Forge player. */
		...
		for (Class<?> type = Connection.class; type != null; type = type.getSuperclass()) {
			for (Field field : type.getDeclaredFields()) {
				...
				try {
					...
					throw new IllegalStateException("无法初始化 1.20.1 Forge AI 网络通道", error);
				}
			}
		}
		throw new IllegalStateException("找不到 1.20.1 Forge 玩家网络通道字段");
	}

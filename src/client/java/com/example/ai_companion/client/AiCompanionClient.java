...
public final class AiCompanionClient implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		MinigameProgress minigameProgress = MinigameProgress.load();
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			chordHeld[0] = pressed;
			boolean minigamePressed = InputConstants.isKeyDown(client.getWindow(), settings.minigameCode());
			if (minigamePressed && !minigameHeld[0] && client.getConnection() != null
					&& client.gui.screen() == null) {
				client.setScreenAndShow(new MinigameCenterScreen(null, minigameProgress));
			}

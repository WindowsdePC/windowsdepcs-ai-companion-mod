		...
		return EclipseUI.configScreen()
			...
			.category(category -> category.name(Component.literal("AI系统"))
				...
				.<AgentMode>dropdown(option -> option.name("默认 AI 模式")
					...
					.defaultValue(AgentMode.SURVIVAL))
			...
			.category(category -> category.name(Component.literal("快捷键修改"))
				...
				.textInput(field -> field.name(Component.literal("AI 消息窗口"))
					.binding(() -> settings.positionsKey, value -> settings.positionsKey = value)
					.defaultValue("F8").maxLength(3).validator(value ->
						value.toUpperCase().matches("F([1-9]|1[0-2])")))
				.textInput(field -> field.name(Component.literal("小游戏中心"))
					.binding(() -> settings.minigameMenuKey, value -> settings.minigameMenuKey = value)
					.defaultValue("M").maxLength(1).validator(value -> value.matches("[A-Za-z]")))
				.label(Component.literal("小游戏方向、动作、暂停、重开和辅助键请在完整九分类中心修改")))
